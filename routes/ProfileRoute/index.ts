...
ProfileRoute.get("/get", async (req, res) => {
  try {
    ...
    if(!paymentAddress) return res.status(400).send({
      success:false,
      message: "paymentAddress parameter is empty."
    })

    const payload = await ProfileModel.findById(paymentAddress);
...
ProfileRoute.get("/getByPaymentAddress", async (req, res) => {
  try {
    ...
    for (const profile of allProfiles) {
      ...
      if (profile.address.length == 0) continue;

      const findedIndex = profile.address.findIndex((address: IAddress, index: number) =>
...
ProfileRoute.post("/saveProfile", async (req, res) => {
  try {
    ...
    if(!walletName) error += 'walletName, ';
    ...
    const payload = await ProfileModel.findOne({
      username
    })
    ...
    let addressArr: any = null;
    const allProfile = await ProfileModel.find();
    allProfile.map((profile: IProfile) => {
    ...
    if (addressArr) {
      const addressFlag = addressArr.find((address: IAddress) => paymentAddress == address.paymentAddress);
    ...
    return res.status(200).send({
      ...
      payload: await ProfileModel.find()
    })
...
ProfileRoute.post("/addWallet", async (req, res) => {
  try {
    ...
    const payloadByUsername = await ProfileModel.findOne({
      username
    })
    ...
    const walletNameIndex = payloadByUsername.address.findIndex((address: IAddress) => address.walletName == walletName);
...
ProfileRoute.post("/removeWallet", async (req, res) => {
  try {
    ...
    const findedIndex = payloadByUsername.address.findIndex((address: IAddress, index: number) =>
      address.paymentAddress == paymentAddress
    )
...
ProfileRoute.post("/renameWallet", async (req, res) => {
  try {
    ...
    const payloadById = await ProfileModel.findById(id);
    ...
    const findedIndex = payloadById.address.findIndex((address: IAddress, index: number) =>
      address.paymentAddress == paymentAddress
    )
...
ProfileRoute.post("/addPoint", async (req, res) => {
  try {
    ...
    const payload = await ProfileModel.findById(id);
